...
class MembershipApplication(models.Model):
    ...
    valid_until = models.DateField(
        null=True, blank=True, help_text="Membership validity date"
    )
    ...
    mobile = models.CharField(
        max_length=20, blank=True, default="", help_text="Format: +880XXXXXXXXXX"
    )
    mobile_number = models.CharField(
        max_length=20, blank=True, help_text="Legacy field"
    )
    ...
    driving_license = models.CharField(
        max_length=3, choices=DRIVING_LICENSE_CHOICES, default="no"
    )
    ...
    occupation = models.CharField(
        max_length=200, choices=OCCUPATION_CHOICES, blank=True
    )
    ...
    number_of_children = models.IntegerField(
        validators=[MinValueValidator(0), MaxValueValidator(20)], default=0, blank=True
    )
    ...
    weight = models.CharField(
        max_length=20, blank=True, help_text='Weight in kg (e.g., "70.5")'
    )
    height = models.CharField(
        max_length=20, blank=True, help_text='Height in feet (e.g., "5\'8"")'
    )
    ...
    proposal_no = models.CharField(
        max_length=100, unique=True, editable=False, blank=True, default=""
    )
    fo_code = models.CharField(
        max_length=50, blank=True, null=True, help_text="Field Officer Code"
    )
    fo_name = models.CharField(
        max_length=200, blank=True, null=True, help_text="Field Officer Name"
    )
    ...
    def generate_proposal_number(self):
        ...
        last_application = (
            MembershipApplication.objects.filter(proposal_number__startswith=prefix)
            .order_by("-proposal_number")
            .first()
        )
...
class Nominee(models.Model):
    ...
    application = models.ForeignKey(
        MembershipApplication, on_delete=models.CASCADE, related_name="nominees"
    )
    ...
    relationship = models.CharField(
        max_length=20, choices=RELATIONSHIP_CHOICES, default="child"
    )
    relation = models.CharField(
        max_length=50, blank=True, help_text="Original relation from frontend"
    )
...
class ApplicationStatusHistory(models.Model):
    ...
    application = models.ForeignKey(
        MembershipApplication, on_delete=models.CASCADE, related_name="status_history"
    )
